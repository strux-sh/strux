...
// Project-level build script step.
// Restricted subset of the BSP lifecycle: project scripts only run during
// rootfs post-processing, operating on the fully assembled rootfs (with a
// managed extract + chroot + in-place repack of rootfs-post.tar.gz) before
// the image is bundled. rootfs_post is the only step today.
const ProjectScriptStepSchema = z.enum([
    "rootfs_post",
])

// Project-level build script schema
const ProjectScriptSchema = z.object({
    // Path to the script file, relative to the project root.
    location: shellSafeRelativePath("scripts.location"),
    // Lifecycle step to run at (defaults to the only supported step).
    step: ProjectScriptStepSchema.default("rootfs_post"),
    // Human-readable description for build logging.
    description: z.string().optional(),
    // Files this script depends on (relative to project) - if any changed, re-run.
    depends_on: z.array(z.string()).optional(),
    // Files generated by this script (relative to dist/) - if all exist and
    // unchanged, the script is skipped. Omit to always run (e.g. "fetch latest").
    cached_generated_artifacts: z.array(z.string()).optional(),
})

export type ProjectScript = z.infer<typeof ProjectScriptSchema>
export type ProjectScriptStep = z.infer<typeof ProjectScriptStepSchema>
