...
// @ts-ignore
import screenViewerHtml from "../../assets/screen-viewer.html" with { type: "text" }
...
export async function dev(): Promise<void> {
    ...
    // Set the screen viewer HTML for the /screen endpoint
    devServer.setScreenViewerHtml(screenViewerHtml)
    Logger.log(`Screen viewer available at http://localhost:${serverPort}/screen`)
...
async function handleConfigAction(action: "restore" | "rebuild-transfer" | "rebuild-builder" | "restart-service" | "reboot"): Promise<void> {
    if (!devUI) return
    ...
    try {
        if (action === "restore") {
            ...
            // If --local-builder was specified, rebuild the Docker image first
            // to pick up any Dockerfile changes (e.g., new dev packages)
            ...
            // Send screen daemon if it exists
            if (await Bun.file(screenPath).exists()) {
                const screenBinary = Buffer.from(await Bun.file(screenPath).arrayBuffer())
                devServer.sendComponent("screen", screenBinary, "/usr/bin/strux-screen")
            }
            ...
            Logger.info("Rebuilding Docker builder image with built-in Dockerfile...")

            // Force rebuild the Docker image using the embedded Dockerfile
            await Runner.prepareDockerImage(undefined, true)

            Logger.success("Docker builder image rebuilt successfully")
            devUI.flashConfigSuccess("Builder image rebuilt")
