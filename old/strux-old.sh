...
# Set dev mode by checking for .dev-env file in /strux
if [ -f /strux/.dev-env ]; then
...
# Use /strux/main for both dev and production
APP_BINARY="/strux/main"

# Determine dev mode based on .dev-env file (already checked earlier)
if [ "$STRUX_IS_DEV" = "1" ]; then
    DEV_MODE=1
    log "Dev mode: Using binary at $APP_BINARY"
else
    DEV_MODE=0
    log "Production mode: Using binary at $APP_BINARY"
fi
...
# Pass STRUX_IS_DEV and DISPLAY_RESOLUTION as environment variables to the subshell
STRUX_IS_DEV="$STRUX_IS_DEV" DISPLAY_RESOLUTION="$DISPLAY_RESOLUTION" cage $SPLASH_ARG -- sh -c '
    echo "[CAGE] Starting cage wrapper script..." > /dev/console

    # Set output resolution using wlr-randr
    echo "[CAGE] Setting display resolution to: $DISPLAY_RESOLUTION" > /dev/console
    wlr-randr --output Virtual-1 --mode "$DISPLAY_RESOLUTION" 2>/dev/null || true
